...
cursor = mydb.cursor()
cursor.execute("USE AirlineManagementSystem")
...
def task1():
	while True:
		...
		if n == "0": 
			break
		if len(n) > 255 or n.isalpha() == False:
			print("Incorrect Entry")
			continue
		#CNIC
		print("Enter CNIC of Passenger: ")
		c = input()
		if c == "0": 
			break
		if len(c)!=13 or c.isdigit()==False:
			print("Incorrect Entry")
			continue
		#PHONE
		...
		if p == "0": 
			break
		if len(p)!=11 or p.isdigit()==False:
			print("Incorrect Entry")
			continue
		#NATIONALITY
		...
		if len(nat) > 255 or nat.isalpha() == False:
			...
			continue
		my_tuple = (n,c,p,nat)
		my_query = "INSERT INTO Passenger (Name, CNIC, Phone, Nationality) Values (%s, %s, %s, %s)"
...
def findPassenger():
	...
	while True:
		...
		if second == "1":
			...
			print("Enter the CNIC")
			marker = input()					
			my_findquery = "SELECT * from Passenger where CNIC = %s"
		elif second == "3":
		...
		x = PrettyTable()
		x.field_names = ["ID", "Name","CNIC", "Phone", "Nationality"]
		for f in record:
			x.add_row(f)
...
def task2():
	...
	while True:
		...
		if u == "1":
			...
			my_updatequery = "Update Passenger set CNIC = %s where PassengerID = %s"
			print("Enter CNIC of Passenger: ")
			value = input()
			if value == "0": 
				break
			if len(value)!=13 or value.isdigit()==False:
				print("Incorrect Entry")
				continue
...
def task6():
	while True:
		...
		if history == []:
			...
			break
		n = PrettyTable()
		n.field_names = ["FlightID", "TicketNum", "PassengerID","Departure_from", "Departure_Date", "Departure_time", "Arrival_at", "Arrival_Date", "Arrival_time", "fare"]
		for h in history:
			n.add_row(h)
		print(n)
		break;
	...
	while True:
		...
		if opt=="2":
			...
			for i in r:
				k.add_row(i)
			print(k)
		print("Enter the Record_no of the record you want to cancel")
		TRID = input()
...
def admintask4():
	while True:
		...
		if len(a) != 3 or a.isalpha() == False:
			...
			continue
		#DATE
		print("Enter Date in the following format: DD/MM/YY")
		ad = input()
		if ad == "0": 
			break
		if len(ad)!=8:
...
def admintask5():
	...
	p = cursor.fetchall()
	y = PrettyTable()
	y.field_names = ["PassengerID", "Name", "CNIC", "Phone", "Nationality"]
	for i in p:
		y.add_row(i)
	print(y)
